			...
			synchronized (SharedInMemoryData.class) {
				if (SHARED_INSTANCE == null) {
					...
					if (BackEndContextListner.isLocalDeployment) {
						...
						config.setProperty("hazelcast.local.localAddress",
						...
						networkConfig.getInterfaces().setEnabled(false)
								.addInterface("OPENSHIFT_JBOSSEWS_IP");
					...
					if (log.isDebugEnabled()) {
						log.debug("Shared Memory Hazelcast instance initiated with INSTANCE_NAME "
								+ INSTANCE_NAME);
					}
				}
			}
