...
public class AdminDAOImpl implements AdminDAO {
	...
	public Admin getAdmin(String userName) {
		...
		try {
			...
			List<Admin> adminAll = session
					.getNamedQuery(Admin.Constants.NAME_QUERY_FIND_BY_USER_NAME)
	...
	public Admin updateAdmin(Admin admin) throws RecordAlreadyModifiedException,
		...
		try {
			...
			if (admin.getLast_modified().equals(
					this.getAdmin(admin.getUser_name()).getLast_modified())) {
				Session session = sessionFactory.getCurrentSession();
				tr = session.beginTransaction();
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest modification of admin " + admin.toString()
							+ " so cannot update");
				}
	...
	public Admin saveAdmin(Admin admin) throws DuplicateRecordException, OperationRollBackException {
		...
		try {
			...
			if (this.isAdminUserNameExist(admin.getUser_name())
					|| this.isAdminUserEmailExist(admin.getEmail())) {
				if (log.isDebugEnabled()) {
	...
	public List<Admin> getAllAdmins() {
		...
		try {
			tr = session.beginTransaction();
			List<Admin> adminAll = session.getNamedQuery(Admin.Constants.NAME_QUERY_FIND_ALL)
					.list();
			tr.commit();
	...
	public boolean validateAdminforUpdateandDelete(Admin admin) {
		...
		if (admin.getId() != null && admin.getUser_name() != null && admin.getUser_name() != ""
				...
				&& admin.getLast_modified() != null && admin.getLast_modified() != "") {
			...
			if (this.isAdminUserNameExist(admin.getUser_name())) {
				...
				status = false;
				if (log.isDebugEnabled()) {
					log.debug("Validation fail due to Admin userName:" + admin.getUser_name()
							+ " doesn't exist");
				}
			}
		} else {
