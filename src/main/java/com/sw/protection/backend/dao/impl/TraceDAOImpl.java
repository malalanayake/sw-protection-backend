...
public class TraceDAOImpl implements TraceDAO {
	...
	try {
	    ...
	    List<Trace> traceAll = session.getNamedQuery(Trace.Constants.NAME_QUERY_FIND_ALL_BY_TYPE_AND_USER_NAME)
		...
		if (log.isDebugEnabled()) {
		    log.debug("Traces are not exist for Type: " + type + " and User name: " + userName);
		}
		return null;
	    } else {
		if (log.isDebugEnabled()) {
		    log.debug("Found " + traceAll.size() + " Traces for Type: " + type + " and User name: " + userName);
		}
		return traceAll;
	    }
	...
	try {
	    tr = session.beginTransaction();
	    List<Trace> traceAll = session.getNamedQuery(Trace.Constants.NAME_QUERY_FIND_ALL_BY_API_NAME)
	...
	try {
	    ...
	    List<Trace> traceAll = session
		    .getNamedQuery(Trace.Constants.NAME_QUERY_FIND_ALL_BY_AFFECTED_TYPE_AND_USER_NAME)
		    .setParameter(Trace.Constants.PARAM_AFFECTED_TYPE_NAME, affectedType)
		    .setParameter(Trace.Constants.PARAM_AFFECTED_TYPE_USER_NAME, affectedUserName).list();
	    tr.commit();

	    if (traceAll.isEmpty()) {
		if (log.isDebugEnabled()) {
		    log.debug("Traces are not exist for affected Type: " + affectedType + " and affected User name: "
			    + affectedUserName);
		}
		return null;
	    } else {
		if (log.isDebugEnabled()) {
		    log.debug("Found " + traceAll.size() + " Traces for affected Type: " + affectedType
