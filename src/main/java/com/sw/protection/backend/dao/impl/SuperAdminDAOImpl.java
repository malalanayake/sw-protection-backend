...
public class SuperAdminDAOImpl implements SuperAdminDAO {
	...
	public List<SuperAdmin> getAllAdmins() {
		...
		try {
			...
			List<SuperAdmin> superAdminAll = session.getNamedQuery(
					SuperAdmin.Constants.NAME_QUERY_FIND_SUPER_ADMIN_ALL).list();
			tr.commit();
	...
	public SuperAdmin getSuperAdmin(String userName) {
		...
		try {
			...
			List<SuperAdmin> superAdminAll = session
					...
					.setParameter(SuperAdmin.Constants.PARAM_SUPER_ADMIN_USER_NAME, userName)
					.list();
			tr.commit();
	...
	@Override
	public void updateSuperAdmin(SuperAdmin admin) throws RecordAlreadyModifiedException,
		...
		try {
			...
			if (admin.getLast_modified().equals(
					this.getSuperAdmin(admin.getUser_name()).getLast_modified())) {
				Session session = sessionFactory.getCurrentSession();
				tr = session.beginTransaction();
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest modification of Super admin "
							+ admin.toString() + " so cannot update");
				}
	...
	@Override
	public void deleteSuperAdmin(SuperAdmin admin) throws RecordAlreadyModifiedException,
			OperationRollBackException {
		Transaction tr = null;
		RecordAlreadyModifiedException recordAlreadyModifiedException = null;
		OperationRollBackException operationRollBackException = null;
		try {
			...
			if (admin.getLast_modified().equals(
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest modification of Super admin "
							+ admin.toString() + " so cannot delete");
				}
				// create custom RecordAlreadyModifiedException
				recordAlreadyModifiedException = new RecordAlreadyModifiedException();
			}
	...
	@Override
	public SuperAdmin saveSuperAdmin(SuperAdmin admin) throws DuplicateRecordException,
			OperationRollBackException {
		Transaction tr = null;
		OperationRollBackException operationRollBackException = null;
