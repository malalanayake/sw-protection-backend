...
public class CompanyUserDAOImpl implements CompanyUserDAO {
	...
	public CompanyUser updateUser(CompanyUser user) throws RecordAlreadyModifiedException,
		...
		try {
			...
			if (user.getLast_modified()
					.equals(this.getUser(user.getUser_name()).getLast_modified())) {
				Session session = sessionFactory.getCurrentSession();
				tr = session.beginTransaction();
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest modification of company user "
							+ user.toString() + " so cannot update");
				}
	...
	public CompanyUser deleteUser(CompanyUser user) throws RecordAlreadyModifiedException,
		...
		try {
			...
			if (user.getLast_modified()
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest modification of company user "
							+ user.toString() + " so cannot delete");
				}
				// create custom RecordAlreadyModifiedException
				recordAlreadyModifiedException = new RecordAlreadyModifiedException();
			}
	...
	public boolean validateCompanyUserforUpdateandDelete(CompanyUser companyUser) {
		boolean status = false;
		if (companyUser.getId() != null && companyUser.getUser_name() != null
				&& companyUser.getUser_name() != "" && companyUser.getEmail() != null
				&& companyUser.getEmail() != "" && companyUser.getLast_modified() != null
				&& companyUser.getLast_modified() != "" && companyUser.getCompany() != null) {
			...
			if (this.isCompanyUserNameExist(companyUser.getUser_name())) {
				...
				status = false;
				if (log.isDebugEnabled()) {
					log.debug("Validation fail due to CompanyUser userName:"
							+ companyUser.getUser_name() + " doesn't exist");
				}
			}
		} else {
			status = false;
