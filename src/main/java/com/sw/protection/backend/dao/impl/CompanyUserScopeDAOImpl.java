...
public class CompanyUserScopeDAOImpl implements CompanyUserScopeDAO {
	...
	public List<CompanyUserScope> getAllCompanyUserScopes(String userName) {
		...
		try {
			...
			List<CompanyUserScope> companyUserScopes = session
					.getNamedQuery(
							CompanyUserScope.Constants.NAME_QUERY_FIND_USER_SCOPE_BY_USER_NAME)
	...
	@Override
	public CompanyUserScope saveNewCompanyUserScope(CompanyUserScope companyUserScope)
	...
	public CompanyUserScope updateCompanyUserScope(CompanyUserScope companyUserScope)
		...
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug("Locked update operation by company user ID "
						+ companyUserScope.getCompanyUser().getId());
			}
			...
			if (companyUserScope.getLast_modified().equals(
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest company user scope:"
							+ companyUserScope.toString() + " so this is not going to upate");
				}
				// create custom RecordAlreadyModifiedException
				recordAlreadyModifiedException = new RecordAlreadyModifiedException();
			...
			if (log.isDebugEnabled()) {
				log.debug("Releasing LOCK by company user ID "
						+ companyUserScope.getCompanyUser().getId());
			}
	...
	public CompanyUserScope getCompanyUserScope(String userName, String apiName) {
		...
		try {
			...
			List<CompanyUserScope> companyUserScopes = session
					.getNamedQuery(
							CompanyUserScope.Constants.NAME_QUERY_FIND_USER_SCOPE_BY_USER_NAME_AND_API_NAME)
					.setParameter(CompanyUserScope.Constants.PARAM_USER_SCOPE_USER_NAME, userName)
					.setParameter(CompanyUserScope.Constants.PARAM_USER_SCOPE_API_NAME, apiName)
					.list();
			tr.commit();
			...
			if (companyUserScopes.isEmpty()) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Company user " + userName
							+ " found with company user scope according to API name " + apiName);
				}
	...
	public boolean isAccessGrantedFor(String userName, String apiName, APIOperations operation) {
		...
		if (companyUserScope == null) {// User doesn't have scope recode for
			// given API
			if (log.isDebugEnabled()) {
				log.debug("Cmmpany user " + userName + " doesn't have scope recode for API "
						+ apiName);
			}
			return result;
		}
		...
		if (log.isDebugEnabled()) {
			log.debug("Company user " + userName + " has scope recode for API " + apiName + " "
					+ operation + "=" + result);
		}
		return result;
	}
		...
		if (companyUserScope != null) {
			if (companyUserScope.getCompanyUser() != null) {
				if (companyUserScope.getCompanyUser().getUser_name() != null
						...
						&& companyUserScope.getCompanyUser().getId() != null
						&& companyUserScope.getApi_name() != null
						&& companyUserScope.getApi_name() != ""
						&& companyUserScope.getLast_modified() != null
