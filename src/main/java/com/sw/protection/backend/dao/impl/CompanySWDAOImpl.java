...
public class CompanySWDAOImpl implements CompanySWDAO {
	...
	public List<CompanySW> getAllCompanySWs() {
		...
		try {
			...
			List<CompanySW> companyUserAll = session.getNamedQuery(
					CompanySW.Constants.NAME_QUERY_FIND_COMPANY_SW_ALL).list();
			tr.commit();
	...
	public CompanySW updateCompanySW(CompanySW companySW) throws RecordAlreadyModifiedException,
		...
		try {
			...
			if (companySW.getLast_modified().equals(
					this.getCompanySW(companySW.getCompany().getUser_name(), companySW.getName())
							.getLast_modified())) {
				Session session = sessionFactory.getCurrentSession();
				tr = session.beginTransaction();
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest modification of company software "
							+ companySW.toString() + " so cannot update");
				}
	...
	public CompanySW deleteCompanySW(CompanySW companySW) throws RecordAlreadyModifiedException,
		...
		try {
			...
			if (companySW.getLast_modified().equals(
				...
				if (log.isDebugEnabled()) {
					log.debug("This is not the latest modification of company software "
							+ companySW.toString() + " so cannot delete");
				}
				// create custom RecordAlreadyModifiedException
				recordAlreadyModifiedException = new RecordAlreadyModifiedException();
			}
	...
	@Override
	public CompanySW saveCompanySW(CompanySW companySW) throws DuplicateRecordException,
			OperationRollBackException {
		Transaction tr = null;
		OperationRollBackException operationRollBackException = null;
	...
	public CompanySW getCompanySW(String companyUserName, String softwareName) {
		...
		try {
			...
			List<CompanySW> companyUserAll = session
					.getNamedQuery(
							CompanySW.Constants.NAME_QUERY_FIND_BY_COMPANY_SW_NAME_AND_COMPANY_USER_NAME)
	...
	public boolean validateCompanySWforSave(CompanySW companySW) {
		...
		if (companySW.getCompany() != null && companySW.getCompany().getUser_name() != ""
				&& companySW.getName() != "") {
			status = true;
		} else {
			status = false;
	...
	public boolean validateCompanySWforUpdateandDelete(CompanySW companySW) {
		boolean status = false;
		if (companySW.getId() != null && companySW.getCompany().getUser_name() != null
				&& companySW.getName() != "" && companySW.getLast_modified() != null
				&& companySW.getLast_modified() != "" && companySW.getCompany() != null) {
			// check whether the data is null or not
			if (this.isCompanySWExist(companySW.getCompany().getUser_name(), companySW.getName())) {
				// check whether the given user name is not changed
				CompanySW saveCompanySw = this.getCompanySW(companySW.getCompany().getUser_name(),
						companySW.getName());
				...
				status = false;
				if (log.isDebugEnabled()) {
					log.debug("Validation fail due to CompanySW Name:" + companySW.getName()
							+ " doesn't exist");
				}
			}
		} else {
