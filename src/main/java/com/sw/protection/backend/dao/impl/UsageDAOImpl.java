...
public class UsageDAOImpl implements UsageDAO {
	...
	public List<UsageData> getAllUsagesByTypeAndID(ObjectType type, Long id) {
		...
		try {
			...
			List<UsageData> usageAll = session
					.getNamedQuery(UsageData.Constants.NAME_QUERY_FIND_ALL_BY_TYPE_AND_ID)
			...
			if (usageAll.isEmpty()) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Found " + usageAll.size() + " Usage for Type: " + type + " and ID: "
							+ id);
				}
				return usageAll;
			}
	...
	public List<UsageData> getAllUsagesByAPIName(String apiName) {
		...
		try {
			tr = session.beginTransaction();
			List<UsageData> usageAll = session
					.getNamedQuery(UsageData.Constants.NAME_QUERY_FIND_ALL_BY_API_NAME)
	...
	@Override
	public void updateUsage(UsageData usage) throws RecordAlreadyModifiedException,
	...
	@Override
	public void deleteUsage(UsageData usage) throws RecordAlreadyModifiedException,
			OperationRollBackException {
		Transaction tr = null;
		RecordAlreadyModifiedException recordAlreadyModifiedException = null;
		OperationRollBackException operationRollBackException = null;
	...
	@Override
	public UsageData saveUsage(UsageData usage) throws DuplicateRecordException,
			OperationRollBackException {
		Transaction tr = null;
		OperationRollBackException operationRollBackException = null;
	...
	public boolean isUsageDataExist(UsageData usage) {
		...
		try {
			tr = session.beginTransaction();
			List<UsageData> usageAll = session
					.getNamedQuery(UsageData.Constants.NAME_QUERY_IS_RECORD_ALREADY_EXIST)
					.setParameter(UsageData.Constants.PARAM_USAGE_API_NAME, usage.getApi_name())
					.setParameter(UsageData.Constants.PARAM_USAGE_API_OPERATION,
							usage.getOperation())
					.setParameter(UsageData.Constants.PARAM_USAGE_TYPE_NAME, usage.getType())
					.setParameter(UsageData.Constants.PARAM_USAGE_TYPE_ID, usage.getType_id())
					.list();
