...
public class CompanyUserServiceImpl implements CompanyUserService {
	...
	public String saveCompanyUser(EncoderDecoderType encoderDecoderType, String companyUserString)
		...
		CompanyUser companyUser = (CompanyUser) decoder.decodeObject(ObjectType.COMPANY_USER,
				companyUserString);
		if (companyUser != null) {
		...
		if (companyUser != null) {
			if (companyUserDAO.validateCompanyUserforUpdateandDelete(companyUser)) {
				if (companyUser.getPass_word() == null || companyUser.getPass_word() == "") {
					...
					CompanyUser companyUserData = companyUserDAO
							.getUser(companyUser.getUser_name());
	...
	@Override
	public String getAllCompanyUsers(EncoderDecoderType encoderDecoderType, int page,
		...
		if (page > 0 && recordePerPage > 0) {
			...
			CompanyUserDAO companyUserDAO = AppContext.getInstance().getBean(CompanyUserDAO.class);
			List<CompanyUser> companyUsers = companyUserDAO.getAllCompanyUsersWithPagination(page,
					recordePerPage);
