...
public class CompanySWCopyServiceImpl implements CompanySWCopyService {
    ...
    @Override
    public String saveCompanySWCopy(EncoderDecoderType encoderDecoderType, String companySWCopyString)
	...
	CompanySWCopy companySWCopy = (CompanySWCopy) decoder.decodeObject(ObjectType.SOFTWARE_COPY,
		companySWCopyString);
	if (companySWCopy != null) {
		...
		if (log.isDebugEnabled()) {
		    log.debug("Given object doesn't contain expected data:" + companySWCopy.toString());
		}
    ...
    @Override
    public String getCompanySWCopy(EncoderDecoderType encoderDecoderType, String clientUserName, String softwareName,
	    String motherBoard, String hd, String mac) throws EncodingException, RequiredDataNotFoundException {
	EncoderFactory encoderFactory = AppContext.getInstance().getBean(EncoderFactory.class);
	DecoderFactory decoderFactory = AppContext.getInstance().getBean(DecoderFactory.class);
	Encoder encoder = encoderFactory.getEncoder(encoderDecoderType);
	CompanySWCopyDAO companySWCopyDAO = AppContext.getInstance().getBean(CompanySWCopyDAO.class);
	...
	if (clientUserName != null && softwareName != null) {
	    companySWCopy = companySWCopyDAO.getCompanySWCopy(clientUserName, softwareName, motherBoard, hd, mac);
	    if (companySWCopy != null) {
		encodedString = encoder.encodeObject(ObjectType.SOFTWARE_COPY, companySWCopy);
	    } else {
    ...
    @Override
    public String deleteCompanySWCopy(EncoderDecoderType encoderDecoderType, String companySWCopyString)
	...
	CompanySWCopyDAO companySWCopyDAO = AppContext.getInstance().getBean(CompanySWCopyDAO.class);
	String encodedString = "";
	CompanySWCopy companySWCopy = (CompanySWCopy) decoder.decodeObject(ObjectType.SOFTWARE_COPY, companySWString);
	if (companySWCopy != null) {
    ...
    @Override
    public String getAllCompanySWCopy(EncoderDecoderType encoderDecoderType, int page, int recordePerPage)
	...
	if (page > 0 && recordePerPage > 0) {
	    ...
	    CompanySWCopyDAO companySWCopyDAO = AppContext.getInstance().getBean(CompanySWCopyDAO.class);
	    List<CompanySWCopy> companySWCopy = companySWCopyDAO
		    .getAllCompanySWCopyWithPagination(page, recordePerPage);
	    if (companySWCopy != null) {
