...
public class CompanyClientServiceImpl implements CompanyClientService {
	...
	public String saveCompanyClient(EncoderDecoderType encoderDecoderType,
		...
		CompanyClientDAO companyClientDAO = AppContext.getInstance()
				.getBean(CompanyClientDAO.class);
		String encodedString = "";

		CompanyClient companyClient = (CompanyClient) decoder.decodeObject(ObjectType.CLIENT,
				companyClientString);
		...
		if (companyClient != null) {
			if (companyClientDAO.validateCompanyClientforUpdateandDelete(companyClient)) {
				if (companyClient.getPass_word() == null || companyClient.getPass_word() == "") {
					...
					CompanyClient companyClientData = companyClientDAO
							.getCompanyClient(companyClient.getUser_name());
	...
	public String getAllCompanyClients(EncoderDecoderType encoderDecoderType, int page,
		...
		if (page > 0 && recordePerPage > 0) {
			...
			CompanyClientDAO companyClientDAO = AppContext.getInstance().getBean(
					CompanyClientDAO.class);
			List<CompanyClient> companyClients = companyClientDAO
					.getAllCompanyClientsWithPagination(page, recordePerPage);
