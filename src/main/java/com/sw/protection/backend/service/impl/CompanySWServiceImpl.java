...
public class CompanySWServiceImpl implements CompanySWService {
	...
	CompanySW companysw = (CompanySW) decoder.decodeObject(ObjectType.SOFTWARE, companySWString);
	if (companysw != null) {
    ...
    @Override
    public String getCompanySW(EncoderDecoderType encoderDecoderType, String companyUserName, String softwareName)
    ...
    @Override
    public String getAllCompanySW(EncoderDecoderType encoderDecoderType, int page, int recordePerPage)
	    throws EncodingException, RequiredDataNotFoundException {
	String encodedString = "";
	...
	if (page > 0 && recordePerPage > 0) {
	    ...
	    CompanySWDAO companySWDAO = AppContext.getInstance().getBean(CompanySWDAO.class);
	    List<CompanySW> companySW = companySWDAO.getAllCompanySWWithPagination(page, recordePerPage);
	    if (companySW != null) {
