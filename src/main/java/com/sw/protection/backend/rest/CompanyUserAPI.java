...
public class CompanyUserAPI {
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Given company user found"),
	...
	try {
		...
		companyUserData = companyUserService.getCompanyUser(EncoderDecoderType.JSON, "{user_name:\"" + userName
			+ "\"}");
	    }
	...
	try {
	    ...
	    // process the JSON type request
	    if (headers.getRequestHeaders().get(ACCEPT_HEADERS).contains(MediaType.APPLICATION_JSON)) {
		companyUserData = companyUserService.getAllCompanyUsers(EncoderDecoderType.JSON, pageNumber,
			itemPerList);
	    }
	    // TODO: Need to process the XML type requests
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Save company user successful"),
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Company user successfuly updated"),
	...
	try {
	    ...
	    // process the JSON type request
	    if (headers.getRequestHeaders().get(ACCEPT_HEADERS).contains(MediaType.APPLICATION_JSON)) {
		companyUserData = companyUserService.updateCompanyUser(EncoderDecoderType.JSON, body);
	    }
	    // TODO: Need to process the XML type requests
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Company user successfuly deleted"),
	...
	try {
	    ...
	    // process the JSON type request
	    if (headers.getRequestHeaders().get(ACCEPT_HEADERS).contains(MediaType.APPLICATION_JSON)) {
		companyUserData = companyUserService.deleteCompanyUser(EncoderDecoderType.JSON, body);
	    }
	    // TODO: Need to process the XML type requests
