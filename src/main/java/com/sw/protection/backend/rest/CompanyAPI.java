...
public class CompanyAPI {
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Given company found"),
	...
	public Response getCompany(
		...
		try {
			...
			if (headers.getRequestHeaders().get(ACCEPT_HEADERS)
				...
				companyData = companyService.getCompany(EncoderDecoderType.JSON, "{user_name:\""
						+ userName + "\"}");
			}
	...
	public Response getCompanyList(
		...
		try {
			...
			if (headers.getRequestHeaders().get(ACCEPT_HEADERS)
					.contains(MediaType.APPLICATION_JSON)) {
				companyData = companyService.getAllCompanies(EncoderDecoderType.JSON, pageNumber,
						itemPerList);
			}
			// TODO: Need to process the XML type requests
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Save company successful"),
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Company successfuly updated"),
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Company successfuly deleted"),
