...
public class AdminScopeServiceImplTest {
	...
	public void saveAdminScope() {
		AdminScopeService adminScopeService = AppContext.getInstance().getBean(
				AdminScopeService.class);
		String adminScopeEncoded = "{\"api_name\":\"admin\",\"get\":true,\"post\":true,\"put\":true,\"del\":true,"
				+ "\"admin\":" + adminReturn + "}";
		String adminScopeReturn = "";
		try {
			adminScopeReturn = adminScopeService.saveAdminScope(EncoderDecoderType.JSON,
					adminScopeEncoded);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void updateAdminScope() {
		...
		JSONEncoder encoder = new JSONEncoder();
		try {
			String adminScopeEncoded = adminScopeService.getAdminScope(EncoderDecoderType.JSON,
					"dinuka", "admin");
			...
			AdminScope decodedAdminScope = (AdminScope) decoder.decodeObject(
					ObjectType.ADMIN_SCOPE, adminScopeEncoded);
			...
			String adminScopeFinalEncoded = encoder.encodeObject(ObjectType.ADMIN_SCOPE,
					decodedAdminScope);
			adminScopeReturn = adminScopeService.updateAdminScope(EncoderDecoderType.JSON,
					adminScopeFinalEncoded);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void deleteAdminScope() {
		...
		try {
			...
			String adminScopeFinalEncoded = encoder.encodeObject(ObjectType.ADMIN_SCOPE,
					decodedAdminScope);
			adminScopeReturn = adminScopeService.deleteAdminScope(EncoderDecoderType.JSON,
					adminScopeFinalEncoded);
			adminScopeEncoded = adminScopeService.getAdminScope(EncoderDecoderType.JSON, "dinuka",
					"admin");
	...
	public void isAccessGrantedFor() {
		...
		String adminScopeReturn = "";
		try {
			adminScopeReturn = adminScopeService.isAccessGrantedFor("dinuka", "user",
		...
		try {
			adminScopeReturn = adminScopeService.isAccessGrantedFor("dinuka", "admin",
					APIOperations.GET.toString());
